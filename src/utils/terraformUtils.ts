...
export async function checkTerraformInstalled(): Promise<void> {
  if (
    settingUtils.isTerminalSetToCloudShell() ||
    !settingUtils.getCheckTerraformCmd()
  ) {
    return;
  }
  try {
    ...
    openUrlHintOrNotShowAgain(
      "Terraform is not installed, please make sure Terraform is in the PATH environment variable.",
